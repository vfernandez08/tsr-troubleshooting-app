...
def generate_recommendations():
    """Generate rule-based troubleshooting recommendations based on collected data"""
    case_id = session.get('case_id')
    if not case_id:
    ...
    try:
        # Get Step 4 and Step 5 data from session
        step4_data = session.get('wifi_env_data', {})
        step5_data = session.get('event_stream_data', {})

        # Simple rule-based recommendations
        recommendations = []

        # Check for common issues based on collected data
        if step4_data.get('eero_only_wifi') == 'yes':
            if step5_data.get('device_band') == '2.4':
                recommendations.append("Switch device to 5 GHz band if supported for better speeds.")

            download_speed = int(step5_data.get('download_speed', 0))
            if download_speed < 50:
                recommendations.append("Power cycle both ONT and eero gateway.")
                recommendations.append("Move device closer to the nearest eero for better signal.")

        if step4_data.get('new_router') == 'yes_24h':
            recommendations.append("Wait 5 minutes for the new node to finish firmware update before testing.")
            recommendations.append("Ensure the new node is >10 ft from the gateway to avoid RF interference.")

        if step4_data.get('ssid_same_as_old') == 'no':
            recommendations.append("Ask customer to 'forget' the old SSID on all devices and reconnect.")
            recommendations.append("Update any IoT devices with the new network name.")

        if 'gateway_offline' in step5_data.get('selected_events', []):
            recommendations.append("Power‑cycle the gateway eero and ONT in proper order (ONT first).")
            recommendations.append("Check the power outlet for loose connections.")

        if 'dfs_strike_detected' in step5_data.get('selected_events', []):
            recommendations.append("Disable DFS channels temporarily in eero app settings.")
            recommendations.append("Use channels 36–48 to avoid DFS interference.")
        ...
        session['recommendation_source'] = 'Rule-based engine'
